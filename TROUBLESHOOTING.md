...
Common issues you might encounter during the workshop and how to resolve them.
...
If you opened a new terminal window or tab since creating the virtual environment, you need to activate it again. The activation is per-terminal-session, not permanent.

## The cache shows MISS every time even though Valkey is running

Check that your `.env` file contains `CACHE_ENABLED=true` (not `True`, not `yes`, not `1`; the code checks for the exact lowercase string `"true"`). Then restart the Flask app. Flask reads environment variables at startup; changing the file without restarting has no effect.
...
It is real for the purposes of this workshop. Valkey responds in sub-millisecond time for simple GET operations, and `time.perf_counter()` rounds to the nearest millisecond in the display. The actual time is a fraction of a millisecond. Compared to the 2,500 ms data source, the difference is effectively instant.
...
If that also fails, Docker may not be installed, or it is not in your PATH. Follow the [Docker installation guide](https://docs.docker.com/get-docker/) for your operating system.
...
It contains completed versions of all the source files at each stage of the workshop. If something goes wrong and you cannot figure out where your code diverged, compare your files against the ones in `safety/`. You can also copy them directly to get back on track:
